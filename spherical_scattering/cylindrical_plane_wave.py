...
@dataclass
class PLANE_WAVE_EXCITATION_TM:
    ...
    def plot_plane_wave(self, title: str = None):
        if title is None:
            title = f"Plane Wave Approximation (TM) n={self.n}"
        else:
            title = title
...
@dataclass
class PLANE_WAVE_EXCITATION_TE:
    h_0: complex  # incidient magnetic field magnitude at the origin point of the plane wave
    k: float  # wave number
    num_samples: int  # number of interpolation points in rho
    rho_max: float
    n: int  # number of sample points to approximation infinite sum for change of basis
    xy_pairs: np.ndarray = field(init=False)
    rho_phi_pairs: np.ndarray = field(init=False)
    h_inc_z_cylindrical: np.ndarray = field(init=False, repr=False)
    ...
    def __post_init__(self):
        ...
        for index, (x, y) in enumerate(self.xy_pairs):
            self.rho_phi_pairs[index] = (np.sqrt(x ** 2 + y ** 2), np.atan2(y, x))
        self.h_inc_z_cylindrical = self.construct_excitation()

    def construct_excitation(self):
        h_inc_z = np.zeros((len(self.rho_phi_pairs)), dtype=complex)
        for index, (rho, phi) in enumerate(self.rho_phi_pairs):
            inc_sum: complex = 0.0j
            for n in range(-self.n, self.n):
                inc_sum += 1j ** (-n) * jvp(n, self.k * rho, 0) * np.exp(1j * n * phi)
            h_inc_z[index] = inc_sum
        e_inc_z = h_inc_z * self.h_0
        return e_inc_z

    def plot_plane_wave(self, title: str = None):
        if title is None:
            title = f"Plane Wave Approximation (TE) n={self.n}"
        else:
            title = title
        #x, y, e_inc_z_cart = cyl_to_cart(self.rho, self.phi, self.e_inc_z_cylindrical)
        X, Y = np.meshgrid(self.x, self.y)
        h_inc_z_cart = np.reshape(self.h_inc_z_cylindrical, (len(self.x), len(self.y)))
        plt.contourf(X, Y, h_inc_z_cart.real)
        plt.colorbar()
        plt.xlabel("x")
        plt.ylabel("y")
        plt.title(title)
...
if __name__ == "__main__":
    ...
    for n in n_vals:
        plane_wave_approx = PLANE_WAVE_EXCITATION_TM(
            e_0=e_0,
            k=k,
    ...
    rho_max = 5.0 * np.sqrt(2)
    for n in n_vals:
        plane_wave_approx = PLANE_WAVE_EXCITATION_TE(
            h_0=h_0,
            k=k,
            num_samples=num_samples,
            rho_max=rho_max,
